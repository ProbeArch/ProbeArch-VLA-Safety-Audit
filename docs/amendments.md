...
- **F1 (blocker, reproduced — FIXED in the final-fix round):** `smoke_test.py`
  ...
  `python3 scripts/audit/shared/smoke_test.py` now prints `SMOKE PASSED` (numpy-only synthetic
...
- (final-fix round, code + docs) **F1 fixed and verified locally:** the two
  ...
  `python3 scripts/audit/shared/smoke_test.py` → `SMOKE PASSED` and `telemetry_rollout.py
