...
- `POLICY_BACKEND=mlx scripts/audit/shared/eval_loop.sh …`
- `python3 scripts/audit/shared/telemetry_rollout.py --device mlx …`
- `python3 scripts/audit/mlx/mlx_smolvla.py --selftest|--probe`

Runtime: [`scripts/audit/mlx/mlx_smolvla.py`](../scripts/audit/mlx/mlx_smolvla.py). Manifests record `policy_backend` and, for MLX, `policy_runtime: probearch-mlx-smolvla`.
...
- `scripts/audit/mlx/mlx_smolvla.py --selftest`
- telemetry / scorer / stats / calibrate self-tests
- `scripts/audit/shared/smoke_test.py` synthetic phase (`SMOKE PASSED`)
...
```bash
python3 scripts/audit/mlx/mlx_smolvla.py --selftest
python3 scripts/audit/shared/telemetry_rollout.py --selftest
python3 scripts/audit/shared/safety_scorer.py --selftest
python3 scripts/audit/shared/stats.py --selftest
python3 scripts/audit/shared/calibrate.py --self-test
...
```bash
python3 scripts/audit/mlx/mlx_smolvla.py --probe --backend mlx
```
...
rm -rf "$AUDIT_DIR"
POLICY_BACKEND=mlx scripts/audit/shared/eval_loop.sh libero_spatial 1 1
```
...
```bash
export AUDIT_DIR=~/audit-mlx
python3 scripts/audit/shared/calibrate.py --suite libero_spatial --task-id 0
python3 scripts/audit/shared/telemetry_rollout.py \
  --device mlx --suite libero_spatial --task_ids 0 --n_envs 1 --n_pairs 1
python3 scripts/audit/shared/safety_scorer.py && python3 scripts/audit/shared/stats.py
```
...
```bash
export AUDIT_DIR=~/audit-mlx
POLICY_BACKEND=mlx scripts/audit/shared/eval_loop.sh libero_spatial 8 4
```
