...
Reviewer: rv_1_shell (fresh context, no prior assumptions). Scope: `scripts/eval_loop.sh`,
`scripts/ship.sh` (core), producers/consumers `scripts/telemetry_rollout.py`,
`scripts/calibrate.py`, `scripts/safety_scorer.py`, `scripts/smoke_test.py`,
`scripts/stats.py`, `scripts/plots.py`, `.gitignore`, `pins.md`, and docs
...
`for task_id in 0 1 2 3 4; do python3 scripts/telemetry_rollout.py --task_ids "$task_id" ...`).
