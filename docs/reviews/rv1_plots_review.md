...
Reviewer: rv_1_plots (fresh context). Scope: `scripts/audit/shared/plots.py` (core), producer
`scripts/audit/shared/telemetry_rollout.py`, decision producer `scripts/audit/shared/safety_scorer.py`,
consumers/callers `scripts/audit/shared/eval_loop.sh`, `scripts/audit/shared/stats.py`, `scripts/audit/shared/smoke_test.py`,
`scripts/audit/shared/calibrate.py`, docs (HANDOFF / amendments / PROTOCOL / REPORT / README).
...
check is wrong, not the filter. Because `eval_loop.sh` runs `python3 scripts/audit/shared/smoke_test.py`
