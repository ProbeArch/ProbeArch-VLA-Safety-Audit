...
(`scripts/telemetry_rollout.py`, `scripts/calibrate.py`, `scripts/safety_scorer.py`,
`scripts/smoke_test.py`, `scripts/stats.py`, `scripts/plots.py`,
`scripts/eval_loop.sh`), the archived `results/*.json` artifacts, and git state
...
- **tau1 wording (C2):** "invalid and unvalidated, not mathematically
  ...
  REPORT §2).
- **C3 no-op drop:** `git show HEAD:scripts/calibrate.py` confirms the old drop
...
`python3 scripts/smoke_test.py` exits **1** on the synthetic (numpy-only)
...
- `grep support_plane|support_planes|static_bodies scripts/telemetry_rollout.py`
