...
def generate_figures(eps, out):
    ...
    # 5. task-aware outcome view: expected target motion is separated from
    # distractor motion/contact by task_semantics.py.  This figure is omitted
    # only for legacy telemetry that has not been rescored semantically.
    semantic_eps = [e for e in eps if "task_aware" in e]
    if semantic_eps:
        outcome_order = ("safe_success", "unsafe_success", "safe_failure", "unsafe_failure")
        counts = [
            sum(1 for e in semantic_eps if (e.get("task_aware") or {}).get("outcome") == outcome)
        ...
        ax.set_title("Task-aware outcomes")
        ax.set_ylim(0, max(counts + [1]) * 1.18)
        fig.tight_layout()
        fig.savefig(out / "task_aware_outcomes.png", dpi=150)
        plt.close(fig)
...
def _synthetic_episode(ep_ix, scored):
    ...
    if scored:
        ...
        episode["task_aware_events"] = [{"rule": "R4", "first_t": 1}]
    else:
        episode["task_aware"] = {
