...
#   (defaults: libero_spatial, 8 pairs, 1 env; flags may appear in any position)
...
#         GENERATE_VIDEOS  replay one representative success/failure per task (default 1)
#         NUM_STEPS        policy diffusion steps (default 1; validated audit pin)
#         N_ACTION_STEPS   policy action horizon (default 1)
#         EPISODE_OFFSET   global episode ID offset (default 0)
set -euo pipefail
...
GENERATE_VIDEOS="${GENERATE_VIDEOS:-1}"
...
case "$NUM_STEPS" in
  ''|*[!0-9]*) echo "NUM_STEPS must be a positive integer, got '$NUM_STEPS'" >&2; exit 2 ;;
esac
case "$N_ACTION_STEPS" in
  ''|*[!0-9]*) echo "N_ACTION_STEPS must be a positive integer, got '$N_ACTION_STEPS'" >&2; exit 2 ;;
esac
case "$EPISODE_OFFSET" in
  ''|*[!0-9]*) echo "EPISODE_OFFSET must be a non-negative integer, got '$EPISODE_OFFSET'" >&2; exit 2 ;;
esac
if [[ "$N_PAIRS" -lt 1 || "$N_ENVS" -lt 1 || "$NUM_STEPS" -lt 1 || "$N_ACTION_STEPS" -lt 1 ]]; then
  echo "N_PAIRS, N_ENVS, NUM_STEPS and N_ACTION_STEPS must be >= 1" >&2
  exit 2
fi
...
if [[ "$GENERATE_VIDEOS" != "0" && "$GENERATE_VIDEOS" != "1" ]]; then
  echo "GENERATE_VIDEOS must be 0 or 1, got '$GENERATE_VIDEOS'" >&2
  exit 2
fi
...
validate_calibration() {
  ...
  "$PYTHON_BIN" - "$path" "$expected_suite" "$expected_task" <<'PY'
import json
import math
import sys

path, expected_suite, expected_task = sys.argv[1:]
try:
    with open(path) as f:
        cal = json.load(f)
...
for key in ("tau1_force_N", "tau2_displacement_m", "tau_tilt_deg"):
    ...
    if not isinstance(value, (int, float)) or not math.isfinite(value) or value <= 0:
        sys.exit(f"{path} missing/invalid {key}: {value!r}")
if cal.get("calibration_suite") != expected_suite:
    sys.exit(f"{path} suite mismatch: {cal.get('calibration_suite')!r}")
if int(cal.get("calibration_task_id", -1)) != int(expected_task):
    sys.exit(f"{path} task mismatch: {cal.get('calibration_task_id')!r}")
print(
    f"calibration OK task={expected_task}: tau1={cal['tau1_force_N']:.3g} N, "
...
validate_calibrations() {
  for task_id in "${TASK_IDS[@]}"; do
    validate_calibration "$CALIBRATION_DIR/${SUITE}_${task_id}.json" "$SUITE" "$task_id"
  done
  if [[ ! -s "$CALIBRATION_DIR/index.json" ]]; then
    echo "calibration index missing/unreadable: $CALIBRATION_DIR/index.json" >&2
    return 1
  fi
}
...
if [[ "$FORCE" == "1" ]]; then
  echo "== --force: discarding $ROLLOUTS_DIR, $CALIBRATION_DIR and stale aggregates =="
  rm -rf "$ROLLOUTS_DIR"
...
echo "== $(date) == eval start: policy=$POLICY backend=$POLICY_BACKEND suite=$SUITE n_pairs=$N_PAIRS n_envs=$N_ENVS num_steps=$NUM_STEPS n_action_steps=$N_ACTION_STEPS episode_offset=$EPISODE_OFFSET n_trials=$N_TRIALS max_trials=$MAX_TRIALS audit_dir=$AUDIT_DIR mode=$MODE"
...
for spec in "shared/telemetry_rollout" "shared/safety_scorer" "shared/stats" "shared/render_videos" "mlx/mlx_smolvla"; do
  echo "== selftest: $spec =="
  "$PYTHON_BIN" "$REPO/scripts/audit/$spec.py" --selftest
done
echo "== selftest: calibrate =="
"$PYTHON_BIN" "$REPO/scripts/audit/shared/calibrate.py" --self-test
echo "== smoke gate =="
"$PYTHON_BIN" "$REPO/scripts/audit/shared/smoke_test.py"

# 1) positive-control calibration -> one scorer-validated profile per task.
#    Skipped on --resume: re-running would change profile hashes and break the
#    run manifest.
if [[ "$RESUME" != "1" ]]; then
  mkdir -p "$CALIBRATION_DIR"
  for task_id in "${TASK_IDS[@]}"; do
    "$PYTHON_BIN" "$REPO/scripts/audit/shared/calibrate.py" \
      --suite "$SUITE" \
      --task-id "$task_id" \
      --out "$CALIBRATION_DIR/${SUITE}_${task_id}.json" \
      --n-trials "$N_TRIALS" \
      --max-trials "$MAX_TRIALS"
  done
  "$PYTHON_BIN" - "$CALIBRATION_DIR" "$SUITE" "${TASK_IDS[@]}" <<'PY'
import hashlib
import json
import os
import sys
...
suite = sys.argv[2]
task_ids = [int(value) for value in sys.argv[3:]]
profiles = []
for task_id in task_ids:
    path = directory / f"{suite}_{task_id}.json"
    profiles.append(
        {
            "suite": suite,
            "task_id": task_id,
...
for task_id in "${TASK_IDS[@]}"; do
  "$PYTHON_BIN" "$REPO/scripts/audit/shared/telemetry_rollout.py" \
    --suite "$SUITE" \
    --task_ids "$task_id" \
    --policy "$POLICY" \
