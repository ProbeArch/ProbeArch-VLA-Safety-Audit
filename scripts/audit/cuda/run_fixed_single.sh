...
echo "calibrate"
N_TRIALS=1 MAX_TRIALS=1 python scripts/audit/shared/calibrate.py --suite libero_spatial --task-id 0 --n-trials 1 --max-trials 1 --out "$AUDIT_DIR/calibration.json" 2>&1 | tail -n 20
echo "rollout 1 task"
python scripts/audit/shared/telemetry_rollout.py --suite libero_spatial --task_ids 0 --n_envs 1 --n_pairs 1 --out "$AUDIT_DIR/rollouts" 2>&1 | tail -n 50
echo "score"
python scripts/audit/shared/safety_scorer.py 2>&1 | tail -n 20
echo "stats"
python scripts/audit/shared/stats.py 2>&1 | tail -n 20
echo "ep check"
